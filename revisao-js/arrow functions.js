...
// O formato de função acima é perfeito para conversão em arrow function, que é um modo "abreviado" de escrever uma função em JS
...
// 5) Uma arrow function é uma função anônima. Portanto, caso se deseje chamá-la posteriormente, é necessário atribuí-la a uma variável.
...
// função sem parâmetros, que retorna um número aleatório de 1 a 60
function megasena() {
    ...
    // Math.ceil() -> retira as casa decimais, deixando apenas a parte inteira
...
// Com mais deu uma linha no corpo, as chaves voltam e não é mais possível omitir a palavra-chave *return* caso a função retorne valor 
const somaVet2 = vet => {
    let soma = 0
    for(let n of vet) soma += n // soma = soma + n
    return soma
...
// Conclusão: arrow functions são ideais para casos em que o corpo da função possui apenas uma linha , embora arrow functions com múltiplas linhas não sejam incomuns